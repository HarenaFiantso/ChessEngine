 ...
 *
 * <p>The project relies on language and library features that only exist on recent JDKs, so a build
 * silently falling back to an older JVM would fail in confusing ways far from the root cause. This
 * test turns the toolchain declaration in {@code build.gradle.kts} into an executable assertion.
 */
