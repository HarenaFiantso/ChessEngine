 ...
 * convention, letting search negate a child's score instead of tracking whose turn it is. Taking a
 * {@link PositionView} lets one evaluator serve both the immutable model and the search's mutable
 * working state.
 */
public interface Evaluator {

  /** Returns the worth of {@code position} in centipawns, positive when the mover stands better. */
  int evaluate(PositionView position);
}
